...
function toUtf8Snippet(content, max = 20_000) {
    if (!content)
        return "";
    if (typeof content === "string")
        return content.slice(0, max);
    if (typeof Buffer !== "undefined" && content.byteLength !== undefined) {
...
export async function runImapIdle(connector, client) {
    if (running.has(connector._id)) {
        console.log("[imap] loop already running for", connector._id);
        return;
    }
    running.add(connector._id);
    const messagesCol = await col("MailMessage");
    const threadsCol = await col("MailThread");
    let backoffMs = 1000;
    ...
    async function initialSync() {
        ...
        for (const uid of newUids) {
            try {
                ...
                const to = (msg.envelope?.to ?? []).map((a) => a.address).filter(Boolean);
                const messageId = msg.envelope?.messageId ?? String(uid);
                const threadId = `${connector._id}:${messageId}`;
                await threadsCol.updateOne({ _id: threadId }, {
                    $setOnInsert: {
                        _id: threadId,
                ...
                const dl = await client.download(String(uid), { uid: true }).catch(() => null);
                const text = toUtf8Snippet(dl?.content);
                await messagesCol.updateOne({ _id: `${threadId}:${uid}` }, {
                    $set: {
                        _id: `${threadId}:${uid}`,
                        tenantId: connector.tenantId,
                        threadId,
    ...
    async function onExists() {
        ...
        if (!latestSeq || typeof latestSeq !== "number")
            return;
        ...
        try {
            ...
            const to = (m.envelope?.to ?? []).map((a) => a.address).filter(Boolean);
            ...
            const dl = await client.download(String(m.uid), { uid: true }).catch(() => null);
            const text = toUtf8Snippet(dl?.content);
            await messagesCol.updateOne({ _id: `${threadId}:${m.uid}` }, {
                $set: {
                    _id: `${threadId}:${m.uid}`,
                    tenantId: connector.tenantId,
                    threadId,
