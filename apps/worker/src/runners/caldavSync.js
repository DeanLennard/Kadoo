...
async function tick() {
    const conns = await (await col("cal_connectors"))
    ...
    for (const c of conns) {
        try {
            if (c.homeUrl) {
                const base = c.homeUrl.startsWith("http")
                    ? c.homeUrl
            ...
            if (!hasSeed) {
                ...
                for (const obj of objs ?? []) {
                    ...
                    await eventsCol.updateOne({ tenantId: c.tenantId, uid: norm.uid }, {
                        $set: {
                            ...norm,
                            connectorId: c._id,
                            rawIcs: obj.data,
                ...
                const newCache = (objs ?? []).map((o) => ({ url: o.url, etag: o.etag }));
                await (await col("cal_connectors")).updateOne({ _id: c._id }, {
                    $set: {
                        cache: newCache,
            ...
            const newCache = [
                ...created.map((o) => ({ url: o.url, etag: o.etag })),
                ...updated.map((o) => ({ url: o.url, etag: o.etag })),
                ...cache.filter((k) => !(deleted ?? []).includes(k.url)),
            ];
            await (await col("cal_connectors")).updateOne({ _id: c._id }, {
                $set: {
