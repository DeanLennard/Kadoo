...
export async function resolveSpamMailbox(client: any): Promise<string | null> {
    ...
    try {
        ...
        if (Array.isArray(listed)) {
            ...
            for await (const b of client.list()) boxes.push(b);
        }
    } catch {
    ...
    const isJunk = (b: any) =>
        String(b?.specialUse || "").toLowerCase() === "\\junk" ||
        (Array.isArray(b?.flags) && b.flags.some((f: any) => String(f).toLowerCase() === "\\junk"));
