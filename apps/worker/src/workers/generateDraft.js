...
function inferConfidence(parsed, msg) {
    const out = (parsed.text || parsed.html || "").trim();
    if (/^\[NO_DRAFT\]$/i.test(out))
        return 0.1;
  ...
  "html": "..."
    ...
    const htmlSignature = settings.signatureHtml?.trim() ||
        (senderName
            ...
            : "");
    const textSignature = senderName
        ? [
            "",
            "",
            "Best regards,",
