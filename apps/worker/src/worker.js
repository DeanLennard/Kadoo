...
import { subscribe, TOPIC_INGEST } from "./queue";
import { col } from "./db";
...
function lockDocFilter(tenantId, threadId, uid, wantVersion) {
    return {
...
subscribe(TOPIC_INGEST, async ({ tenantId, threadId, uid }) => {
    ...
    // try to acquire lock
    const wantVersion = 1;
    const updated = await messages.findOneAndUpdate(lockDocFilter(tenantId, threadId, uid, wantVersion), { $set: { processingLock: { at: now, expiresAt: new Date(now.getTime() + LOCK_TTL_MS) } } }, { returnDocument: "after" });
    // Support either ModifyResult<T>.value or direct doc-or-null
    const lockedDoc = updated?.value !== undefined ? updated.value
        : updated;
    if (!lockedDoc) {
    ...
    await messages.updateOne({ tenantId, threadId, uid }, { $set: { classifiedAt: new Date(), classificationVersion: wantVersion }, $setOnInsert: {}, $unset: { processingLock: "" } });
    // thread aggregates
    const threadLabels = Array.from(new Set([...(facts.labels || [])]));
    await threads.updateOne({ _id: threadId, tenantId }, { $set: { labels: threadLabels, lastClassifiedAt: new Date(), priority: facts.priority ?? 1 } });
            ...
            try {
                ...
                if (dest) {
                    try {
                        ...
                        // Reflect move in DB
                        await messages.updateOne({ tenantId, threadId, uid }, { $set: { folder: dest }, $addToSet: { labels: "spam" } });
                        await threads.updateOne({ _id: threadId, tenantId }, { $set: { folder: dest, lastUid: uid }, $addToSet: { labels: "spam" } });
                        ...
                        await messages.updateOne({ tenantId, threadId, uid }, { $addToSet: { labels: "spam" } });
